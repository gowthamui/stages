myApp.directive('surveyDateWidget', ['$filter','$rootScope','$log','ThemeService','UserService','DataService','$state', function($filter,$rootScope,$log,ThemeService,UserService,DataService,$state) {
        return {
            restrict: 'A',
            replace: true,
            terminal:true,
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                scope.$watch('valueSummary.dateValue', function (newValue,oldValue) {
                    try {
                        var d = $filter('date')(scope.valueSummary.dateValue, 'yyyy-MM-dd')
                        element.val(d);
                    } catch (x) {
                    }
                });
