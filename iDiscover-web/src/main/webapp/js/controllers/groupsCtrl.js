myApp.controller('GroupsCtrl', ['$scope','$rootScope','DataService','$stateParams','$modal', function ($scope,$rootScope,DataService,$stateParams,$modal) {
	$scope.title='Groups';
	$scope.selectedItems=[];
	$scope.gridData = [];
	...
	$scope.gridOptions = {
		...
		columnDefs: [
			...
			{field: 'isDisabled', displayName: 'Disabled?',cellTemplate:closedTemplate,width:100}
	...
	$scope.openAddEdit=function(itemToEdit){
		var d = $modal.open({
			templateUrl:'../partials/editGroups.html',
	...
	$scope.refreshRows=function(){
		...
		DataService.post('data/Groups/search',{showClosedItems:$scope.showClosed}).then(function(result){
